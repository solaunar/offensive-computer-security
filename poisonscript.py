...
"""
def getmac(ip):
	logging.info("Getting MAC Address for IP: {ip}".format(ip = ip))
	mac = None
	tries = 1
	while (mac == None and tries < 10):
		...
		tries += 1
	if (mac == None):
		raise Exception("MAC Address could not be obtained for IP: {ip}".format(ip = ip))
	return mac
...
def poisonarp(targetip, targetmac, sourceip):
	logging.debug("Poisoning ARP with Target IP: {targetip}, Target MAC: {targetmac}, \
		Source IP: {sourceip}".format(targetip, targetmac, sourceip))
...
def restorearp(targetip, targetmac, sourceip, sourcemac):
	logging.debug("Restoring ARP with Target IP: {targetip}, Target MAC: {targetmac}, \
		Source IP: {sourceip}, Source MAC: {sourcemac}".format(targetip, targetmac, sourceip, sourcemac))
